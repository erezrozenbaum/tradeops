    ...
    def test_minor_always_gets_full_low_risk(self):
        for modifier in ("reduce", "neutral", "allow_growth"):
            for level in ExperienceLevel:
                result = _compute_allocation(modifier, level, age_tier="minor")
...
class TestReduceModifier:
    ...
    def test_reduce_caps_high_risk_at_zero(self):
        for level in ExperienceLevel:
            _, _, high, _ = _compute_allocation("reduce", level, age_tier="adult")
            assert high == 0.0, f"reduce/{level} should have 0% high risk"

    def test_reduce_low_risk_dominant(self):
        for level in ExperienceLevel:
            low, growth, high, _ = _compute_allocation("reduce", level, age_tier="adult")
            assert low == 85.0
            assert growth == 15.0
            assert high == 0.0
    ...
    def test_reduce_max_drawdown_is_5(self):
        for level in ExperienceLevel:
            _, _, _, drawdown = _compute_allocation("reduce", level, age_tier="adult")
            assert drawdown == 5.0
...
class TestNeutralModifier:
    def test_neutral_beginner(self):
        low, growth, high, drawdown = _compute_allocation(
            "neutral", ExperienceLevel.beginner, age_tier="adult"
        )
        assert low == 70.0
        assert growth == 25.0
        assert high == 5.0
    ...
    def test_neutral_intermediate(self):
        low, growth, high, drawdown = _compute_allocation(
            "neutral", ExperienceLevel.intermediate, age_tier="adult"
        )
        assert low == 60.0
        assert growth == 30.0
        assert high == 10.0
    ...
    def test_neutral_advanced(self):
        low, growth, high, drawdown = _compute_allocation(
            "neutral", ExperienceLevel.advanced, age_tier="adult"
        )
        assert low == 50.0
        assert growth == 35.0
        assert high == 15.0
...
class TestAllowGrowthModifier:
    def test_allow_growth_beginner(self):
        low, growth, high, drawdown = _compute_allocation(
            "allow_growth", ExperienceLevel.beginner, age_tier="adult"
        )
        assert low == 50.0
        assert growth == 35.0
        assert high == 15.0
    ...
    def test_allow_growth_intermediate(self):
        low, growth, high, drawdown = _compute_allocation(
            "allow_growth", ExperienceLevel.intermediate, age_tier="adult"
        )
        assert low == 40.0
        assert growth == 40.0
        assert high == 20.0
    ...
    def test_allow_growth_advanced(self):
        low, growth, high, drawdown = _compute_allocation(
            "allow_growth", ExperienceLevel.advanced, age_tier="adult"
        )
        assert low == 30.0
        assert growth == 45.0
        assert high == 25.0
...
class TestAllocationInvariants:
    ...
    def test_all_non_minor_combinations_sum_to_100(self):
        for modifier in ("reduce", "neutral", "allow_growth"):
            for level in ExperienceLevel:
                low, growth, high, _ = _compute_allocation(modifier, level, age_tier="adult")
    ...
    def test_experience_increases_growth_allocation_for_neutral(self):
        beginner_high = _compute_allocation("neutral", ExperienceLevel.beginner, "adult")[2]
        intermediate_high = _compute_allocation("neutral", ExperienceLevel.intermediate, "adult")[2]
        advanced_high = _compute_allocation("neutral", ExperienceLevel.advanced, "adult")[2]
        assert beginner_high < intermediate_high < advanced_high

    def test_allow_growth_more_aggressive_than_neutral_same_level(self):
        for level in ExperienceLevel:
            _, _, neutral_high, _ = _compute_allocation("neutral", level, "adult")
            _, _, growth_high, _ = _compute_allocation("allow_growth", level, "adult")
