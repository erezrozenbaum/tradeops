...
| Paper Position Price History | `paper_trading/router.py` → `GET /paper-portfolios/{id}/positions/{pid}/price-history?period=1m\|3m\|6m` | Fetches real daily closes from Yahoo Finance for a paper position's symbol filtered to dates ≥ entry date; returns entry_price, points with per-day return_pct, current_price, total_return_pct. Frontend: "Chart" toggle per position card → inline SVG line chart with dashed entry-price reference line, period selector (1m/3m/6m), results cached client-side. No DB migration. Applied v3.22.0. |
| Next.js 16 Upgrade + Security | `frontend/package.json` + route handlers + `layout.tsx` | Upgraded Next.js 14 → 16.2.6 (Turbopack), ESLint 8 → 9, eslint-config-next 16, postcss ^8.5.10. Resolves 9 CVEs (SSRF CVSS 8.6, DoS, HTTP smuggling, XSS, cache poisoning). Breaking changes migrated: `params` in route handlers now `Promise<{...}>` (awaited); dashboard layout requires `"use client"` for `ssr: false` dynamic imports. Applied v3.23.0. |
| Light Mode Fix | `globals.css` + `sidebar.tsx` + `layout.tsx` | Replaced hardcoded dark HSL inline styles with CSS variables (`--sidebar-from`, `--sidebar-to`, `--sidebar-border`, `--sidebar-divider`, `--topbar-bg`, `--topbar-border`); light-mode values added to `.light` block; scrollbar and `--narrative-*` also theme-aware; nav hover replaced `cyber-rule` with `muted`. Applied v3.23.0. |
| Trade Journal | `staged_orders/` + migration 0054 + `/journal` page | Decision capture and post-execution reflection on every staged order. `rationale TEXT` + `reflection JSONB` on `staged_orders`. `PATCH /{order_id}/rationale` to add/edit. `GET /journal` returns `JournalEntryOut` list with pre-flight verdict, rationale, and reflection. Reflection computed on `mark_executed`: records preflight verdict, flagged risks, whether rationale was documented. Order Builder gains "Why this trade?" textarea. Paper Trading promote flow shows rationale capture modal. New `/journal` page with stats, filters, inline rationale editing, reflection cards. Applied v3.24.0. |
| Decision Intelligence | `decision_intelligence/` + `/decision-intelligence` page | Decision Quality Score (DQS) 0–100 measuring process quality independent of market performance. Four components: Documentation Discipline (0–35), Risk Intelligence (0–30), Goal Alignment (0–20), Outcome Correlation (0–15). Outcome Correlation uses live price cache (current price vs. entry price on executed buy orders) — no extra data entry. Monthly DQS history with trend detection. Behavioral insight cards (up to 6) categorised as strength / warning / pattern / opportunity. Coach notes (2–3 data-derived nudges). `GET /investors/{id}/decision-intelligence`. Applied v3.25.0. |
| Paper Trading UX | `paper_trading/service.py` + paper-trading page | `reprice_positions()` now runs for all portfolio statuses (active and completed). Reprice button always visible. "+ Buy more" quick button pre-fills trade form per position. Symbol field updated with crypto hint (`AAPL / BTC / ETH`). Positions without cached price show dashed `no price — hit Reprice` badge. Applied v3.25.0. |
...
| Behavioral Confidence Indicator (v3.30.0) | `app/services/behavioral_indicator.py` + `staged_orders/schemas.py` + `staged_orders/service.py` + `PreFlightBehavioralShield.tsx` | Read-only κ score (0–1) embedded in every staged order's pre_flight_review JSONB. Inputs: DQS (normalized 0–1), documentation alpha (±25pp clamped, scaled to ±0.125), override ratio (×0.25 penalty), thesis penalty (0.15 if no rationale), asset edge penalty (0.10 if <5 buys with positive avg return). Four tiers: HIGH_ALPHA / STANDARD / CAUTION_IMPULSE / HIGH_RISK_OVERRIDE + INSUFFICIENT_DATA. Requires ≥5 executed orders to produce a signal. Frontend: ShieldCheck (green), ShieldAlert (amber/rose), ShieldQuestion (grey) chip in expanded order view. Never modifies order size or risk engine verdict. |
| Redis Caching (v3.33.0) | `app/core/cache.py` + `decision_intelligence/router.py` + `behavioral_alpha/router.py` + `reflection_report/router.py` + `staged_orders/router.py` + `staged_orders/service.py` | Redis caching for the four compute-heavy read endpoints. `REDIS_URL` env var must be set (already wired in docker-compose). TTL: 900s for DI, BA, calibration; 1800s for reflection report. Cache keys: `di:{id}`, `ba:{id}`, `cal:{id}`, `rr:{id}:{YYYY-MM}`. `invalidate_investor()` uses `scan_iter` for non-blocking pattern key deletion. Called in service layer after create/execute/cancel order mutations. All cache operations wrapped in try/except — Redis unavailability silently degrades to uncached behavior. |
| Thesis Expiry Monitor (v3.32.0) | `app/services/thesis_drift.py` + `staged_orders/schemas.py` + `morning_brief/router.py` + Morning Brief page + Order Builder page | `thesis_params` JSONB (migration 0055) stores optional stop-loss %, take-profit %, and horizon days per staged buy order. `get_thesis_alerts()` runs on every Morning Brief request, evaluates all executed buy orders with thesis_params: RISK_BREACHED (price ≤ entry + stop_loss_pct), TAKE_PROFIT_REACHED (price ≥ entry + take_profit_pct), TIMELINE_EXPIRED (days held > horizon_days), INSUFFICIENT_DATA (no price snapshot). Results appear in Morning Brief as a thesis alert card with per-order status chips and return % vs entry. Order Builder: collapsible "Thesis parameters" section in create form; sky-colored chip on order cards showing configured thresholds. Isolated in try/except — never breaks the Morning Brief. |
| Portfolio Anti-Correlation Engine (v3.31.0) | `app/services/correlation_engine.py` + `staged_orders/schemas.py` + `staged_orders/service.py` + `PreFlightDiversificationCard.tsx` | Pearson correlation between a staged buy ticker and the investor's top 5 holdings by estimated value. Result embedded in `pre_flight_review.diversification` JSONB. Risk tiers: HIGH_OVERLAP (avg r ≥ 0.70 — amber), MODERATE_OVERLAP (0.30–0.70 — neutral), HIGHLY_DIVERSIFIED (<0.30 — emerald). Per-ticker correlation breakdown shown as color-coded chips. MIN_HISTORICAL_DAYS=15 guard returns INSUFFICIENT_DATA if price history is too short. SKIPPED for orders without a ticker or sell orders. Uses `price_snapshots` table — deduplicates to one price per calendar day. Never modifies verdict or order sizing. |
| Smart Assist v2 (v3.29.0) | `staged_orders/smart_suggest.py` | `_build_context()` now injects `dqs_score` (all-time DQS) and `mistake_patterns` (from Behavioral Alpha) into the AI prompt; rules direct the model to address low DQS, position sizing for reactive trades, risk compliance for blind overrides, and portfolio review for undocumented losses; deterministic fallback adds DQS discipline nudge (<50) and pattern-specific suggestions. |
| SIP Price-Alert Triggers (v3.29.0) | `recurring_plans/schemas.py` + `workers/jobs/price_alert_sip_trigger.py` + `scheduler.py` + recurring-plans UI | `PlanAllocation.trigger_on_alert: bool = False` added (JSONB, no migration); when enabled, the allocation's ticker fires a buy order when a matching price alert triggers; `price_alert_sip_trigger.py` worker runs at 20:45 UTC daily; deduplication via `[alert_trigger:{id}]` in order notes; Bell badge shown on trigger-linked allocations in read-only view. |
| Next.js cleanup (v3.29.0) | `frontend/src/app/(dashboard)/help/page.tsx` | Removed unnecessary `"use client"` directive from the Help page — page uses no client APIs; now a proper Server Component reducing JS bundle for that route. |
| Active Broker Sync Warnings (v3.28.0) | `broker_sync/status.py` + `morning_brief/router.py` + `staged_orders/service.py` + Morning Brief page | `get_outdated_accounts()` helper returns all accounts with stale (25h+) or outdated (72h+) sync status. Morning Brief response includes `broker_sync_warnings[]` list with account name, provider, sync_status, and last_synced_at; displayed as an amber/red card. Pre-flight review on order creation now adds a "Stale broker data" risk entry for any outdated (72h+) account, influencing verdict. Applied v3.28.0. |
| Outcome Calibration Dashboard (v3.28.0) | `staged_orders/service.py` + `staged_orders/schemas.py` + `staged_orders/router.py` + `/outcome-calibration` page | Aggregates `outcome_snapshots` from executed orders to compare projected vs actual tier allocations at 30/90/180-day milestones. `GET /investors/{id}/staged-orders/calibration` returns `CalibrationOut` with per-milestone stats (order_count, avg projected/actual for each tier, avg accuracy score) and per-order rows. Accuracy = 100 − avg(|proj_tier% − act_tier%|). Filterable order detail table. Added to Intelligence sidebar section. Applied v3.28.0. |
